...
#
# Obs. Essa implementação é referente ao caso específico, onde omega +- 0.5
...
class A_MLP:
    ...
    def backward(self, y_truth, c_error, phi, omega, print_mse=False):
        ...
        for i, y in enumerate(self.outputs[-1]):
            ...
            error_prime = -(y_truth[i] - y)*phi 
            
            # Parâmetro flexível, alternando conforme sinal de error_prime (a primeira medida ponderada)
            if error_prime < 0:
                error_prime -= omega * c_error
            elif error_prime > 0:
                error_prime += omega * c_error
