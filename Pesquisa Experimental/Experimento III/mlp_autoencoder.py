# Implementação do modelo Multilayer Perceptron (MLP) para remoção de ruído de dígitos MNIST
#
# Baseado em arquitetura autoencoder, inspirada em Ekman (2021)
# 
# Detalhes do modelo:
...
#
# * O código é basicamente o MLP para a classificação. Nenhuma adaptação extrema é necessária, uma vez
#   e possível utilizar o mesmo procesos de treino para o caso não supervisionado. Nessa situação,
#   o que muda é a invocação da função de treino. Aqui o MSE aproximado indica uma medida de otimização
#   para a difença entre os pixels de uma imagem ruidosa e uma imagem nítida.
#
#   Obs. Outra alteração feita é o descarte do processo de validação. 
